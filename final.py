...
class Trader:
    ...
    def extract_bids(self, orders, product, bids, price, gap, depth, vol, pos):
        did_sell = 0
        for i in bids:
            if i <= price - gap:
                bid_vol = max(-depth.buy_orders[i], vol - pos - did_sell)
                if bid_vol < 0:
                    orders.append(Order(product, i, bid_vol))
                    did_sell += bid_vol
    
    def extract_asks(self, orders, product, asks, price, gap, depth, vol, pos):
        did_buy = 0
        for i in asks:
            if i >= price + gap:
                ask_vol= min(-depth.sell_orders[i], -vol - pos - did_buy)
                if ask_vol > 0:
                    orders.append(Order(product, i, ask_vol))
                    did_buy += ask_vol